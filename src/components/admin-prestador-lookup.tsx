...
export function PrestadorLookup() {
  ...
  const findManualPageForPrestador = (prestadorName: string): string | null => {
    ...
    const upperPrestadorName = prestadorName.toUpperCase();
    let keywordsToSearch: string[] = [];

    if (upperPrestadorName.includes("VIVO")) {
      keywordsToSearch = ["TELECOMUNICAÇÕES", "TELECOMUNICAÇÃO"];
    } else if (upperPrestadorName.includes("ENEL")) {
      keywordsToSearch = ["ENERGIA", "ENERGIA ELETRICA"];
    } else {
      return null;
    }
    ...
    for (const notebook of manuals) {
      for (const page of notebook.pages) {
        const upperPageTitle = page.title.toUpperCase();
        for (const keyword of keywordsToSearch) {
          if (upperPageTitle.includes(keyword)) {
            return page.id;
          }
        }
      }
